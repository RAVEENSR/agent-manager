...
> **Stale names:** the `obs-gateway-*` ConfigMap and deployments named below belong to an older platform layout. For the current resource names — and a combined fix that also covers the deployed-agent API-key 401 — see §12.
...
### 12. Deployed-agent calls 401, and traces 401: gateway can't reach `host.docker.internal`

Two symptoms, one root cause. Either calling a deployed agent returns:

```
401 {"error":"Unauthorized","message":"Valid API key required"}
```

or the agent's pod logs show every trace export failing:
...
The api-platform gateway has `host.docker.internal` baked into several config values. That alias only resolves under Docker Desktop — **k3d on Colima resolves `host.k3d.internal` instead** (same root cause as §10). Two independent paths break:

- **Control channel — breaks API-key auth.** The gateway controller dials `agent-manager-service` over a WebSocket (`APIP_GW_CONTROLPLANE_HOST`). With `host.docker.internal` the dial fails:

  ```
  WebSocket connection failed: dial tcp: lookup host.docker.internal
    on 10.43.0.10:53: no such host
  ```

  API keys are pushed to the gateway over that WebSocket. `agent-manager-service` persists the key to its DB *before* broadcasting and treats "no gateway connected" as success (`broadcast api key event; no active connections … skipping`). So the Console hands you a valid-looking key the gateway never received, and every call to the agent 401s — regardless of which key you send.

- **JWT validation — breaks trace ingest.** The `/otel` route uses `jwt-auth`; the gateway fetches the issuer's JWKS from `host.docker.internal:9000`. Unreachable JWKS means every telemetry token is rejected, so trace exports 401.

Confirm the controller side:

```bash
kubectl logs -n openchoreo-data-plane \
  deploy/api-platform-default-default-gateway-controller | grep -i "control plane"
# bad: websocket_url wss://host.docker.internal:9243/... / "no such host"
```

and the broadcast side:

```bash
docker logs agent-manager-service 2>&1 | grep "no active connections"
```

**Fix — point every gateway reference at `host.k3d.internal`:**

```bash
NS=openchoreo-data-plane

# 1. Controller's control-plane host (the WebSocket target).
kubectl set env deploy/api-platform-default-default-gateway-controller -n $NS \
  APIP_GW_CONTROLPLANE_HOST=host.k3d.internal:9243

# 2. The config maps that still carry the alias (JWKS URI + control-plane host).
for cm in api-platform-default-default-config \
          api-platform-default-default-gateway-config \
          gateway-operator-gateway-values; do
  kubectl get cm "$cm" -n $NS -o json \
    | python3 -c "import json,sys; d=json.load(sys.stdin); d['data']={k:v.replace('host.docker.internal','host.k3d.internal') for k,v in d.get('data',{}).items()}; print(json.dumps(d))" \
    | kubectl apply -f -
done

# 3. Restart both gateway deployments.
kubectl rollout restart deploy/api-platform-default-default-gateway-controller      -n $NS
kubectl rollout restart deploy/api-platform-default-default-gateway-gateway-runtime -n $NS
```

Verify the WebSocket comes up:

```bash
kubectl logs -n openchoreo-data-plane \
  deploy/api-platform-default-default-gateway-controller | grep "Connection state" | tail -1
# expect: ..."to":"connected"

docker logs agent-manager-service 2>&1 | grep "Gateway connected" | tail -1
# expect: Gateway connected … totalConnections=1
```

On reconnect the controller bulk-syncs every API-key hash. Keys minted while the WebSocket was down were only persisted, never delivered — **issue a fresh key from the Console's Try-It panel**; older keys (including any cached in the browser) stay 401. Then send a chat message and confirm `/otel/v1/traces` returns 200:

```bash
kubectl logs -n openchoreo-data-plane \
  deploy/api-platform-default-default-gateway-gateway-runtime | grep "otel/v1/traces" | tail
```

The `kubectl set env` patch survives pod restarts but not `helm upgrade` or a fresh `make setup`. A current `make setup` writes the right host via `setup-openchoreo.sh`; this procedure is only for setups created before that fix.

> **Supersedes §10's resource names.** §10 calls the trace-ingest gateway `obs-gateway` (ConfigMap `obs-gateway-config`, deployments `obs-gateway-gateway-controller` / `obs-gateway-gateway-gateway-runtime`). Those names are from an older platform layout. On the current platform the `/otel` route is served by the **same** api-platform gateway as agent traffic — use the `api-platform-default-default-*` resources above. §10's diagnosis (unreachable JWKS) is otherwise correct.
