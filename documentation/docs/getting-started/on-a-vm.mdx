...
The installer runs in three phases — bootstrap (Docker + firewall), a preflight reachability check for `:443`, then the platform install and Caddy startup. Allow 8–15 minutes.
...
- **Preflight fails on :443** — open inbound `:443` in your cloud security group / NACL, and make sure the public `:443` reaches the VM as **raw TCP**: a TLS-terminating load balancer in front breaks the TLS-ALPN-01 challenge.
