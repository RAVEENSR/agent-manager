...
```bash
curl -fsSL https://wso2.github.io/agent-manager/install.sh | AMCTL_VERSION=1.0.0-beta AMCTL_INSTALL_DIR="$HOME/bin" sh
```
:::

### Windows (x64)

Download the release archive from the [Agent Manager releases page](https://github.com/wso2/agent-manager/releases). The commands below use **`1.0.0-beta`** as an example — replace it with the latest version.

```powershell
Invoke-WebRequest -Uri https://github.com/wso2/agent-manager/releases/download/amp/v1.0.0-beta/amctl_v1.0.0-beta_windows_amd64.zip -OutFile amctl.zip
